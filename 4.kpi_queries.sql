...
ORDER By  sum (cost_price) > 100 DESC ;
 ...
 Window Fuction : ROW_NUMBER() 
\\KPI 12 Nummeriere alle Bestellungen pro Kunde nach Datum (neueste = 1)
SELECT
    customer_id,
    order_id,
    order_date,
...
\\KPI 13:Ranke Produkte nach Revenue (höchster Revenue = Rang 1)
SELECT
    product_id,
    revenue,
...
\\KPI 14 : Gesamtumsatz pro Produkt
SELECT product_id, SUM(revenue) AS total_revenue
FROM fact_order_items
GROUP BY product_id;

\\KPI 15 : Nur Produkte mit Gesamtumsatz über 500
SELECT product_id, SUM(revenue) AS total_revenue
FROM fact_order_items
GROUP BY product_id
HAVING SUM(revenue) > 500;

\\KPI 16 :  Produktname + Gesamtumsatz (JOIN mit dim_product)
...
\\KPI 17: Ranke Kunden nach Anzahl ihrer Bestellungen (DENSE_RANK)
SELECT
    customer_id,
    COUNT(*) AS total_orders,
...
\\KPI 18. Durchschnittliche Lieferzeit pro Bestellung
SELECT AVG(JULIANDAY(delivery_date) - JULIANDAY(shipped_date)) AS avg_delivery_days
FROM shipments
WHERE delivery_date IS NOT NULL;

\\KPI 19: Wie viele Bestellungen pro Status?
SELECT status, COUNT(*) AS total
FROM orders
GROUP BY status;

\\KPI 20 : Zeige Kunden + deren Bestellstatus (JOIN 3 Tabellen)
